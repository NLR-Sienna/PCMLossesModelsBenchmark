...
# Extract per-unit bus injections from the solved model in PTDF bus-axis order.
function get_ptdf_bus_injections(model)
    container = PSI.get_optimization_container(model)
    optimize!(PSI.get_jump_model(container))  # PSI clears JuMP solution on solve!; re-optimize to restore
