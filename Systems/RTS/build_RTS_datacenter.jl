...
function candidate_projects_data(sys)
    ...
    gen4 = get_component(Generator, sys, "322_CT_6")
    ...
    return [
        ...
        ThermalStandard( 
            ...
            status = gen4.status,
            bus = get_component(ACBus, sys, "Baker"),
...
function add_candidate_line_data!(sys)
    line1 = get_component(Line, sys, "CA-1")
    line2 = get_component(Line, sys, "AB-2")
    line3 = get_component(Line, sys, "CB-1")
    new_candidates = [
        Line(
        ...
        Line(
            name = "candidate_line_3",
            available = line3.available,
            active_power_flow = line3.active_power_flow,
            reactive_power_flow = line3.reactive_power_flow,
            arc = line3.arc,
            r = line3.r,
            x = line3.x,
            b = line3.b,
            rating = line3.rating,
            angle_limits = line3.angle_limits,
            ext = Dict(
                "is_candidate" => true,
                "project_cost" => 285.4,
            ),
...
function add_candidate_line_data_without_parallel!(sys)
    candidate_info = [
        ("CA-1", "candidate_line_1", Dict{String, Any}("is_candidate" => true, "project_cost" => 285.4)),
        ("AB-2",  "candidate_line_2", Dict{String, Any}("is_candidate" => true, "project_cost" => 285.4)),
        ("CB-1",  "candidate_line_3", Dict{String, Any}("is_candidate" => true, "project_cost" => 285.4)),
    ]
...
#     bus = get_component(ACBus, sys, "Baker")
#     load = get_component(PowerLoad, sys, "Baker")
...
# function add_candidate_datacenter_line_data!(sys)
#     line1 = get_component(Line, sys, "bus-4-bus-10-i_7")
#     arc1 = Arc(; from = get_component(ACBus, sys, "datacenter_bus"), to = get_component(ACBus, sys, "bus-10"))
#     line2 = get_component(Line, sys, "bus-1-bus-4-i_2")
#     arc2 = Arc(; from = get_component(ACBus, sys, "bus-1"), to = get_component(ACBus, sys, "datacenter_bus"))
#     line3 = get_component(Line, sys, "bus-1-bus-2-i_1")
#     arc3 = Arc(; from = get_component(ACBus, sys, "bus-4"), to = get_component(ACBus, sys, "datacenter_bus"))
#     transformer1 = get_component(PhaseShiftingTransformer, sys, "bus-3-bus-4-i_5")
#     arc4 = Arc(; from = get_component(ACBus, sys, "bus-3"), to = get_component(ACBus, sys, "datacenter_bus"))
#     new_candidates_line = [
#         Line(
#             name = "candidate_datacenter_line_1",
#             available = line1.available,
#             active_power_flow = line1.active_power_flow,
#             reactive_power_flow = line1.reactive_power_flow,
#             arc = arc1,
...
#         ),
#         Line(
#             name = "candidate_datacenter_line_2",
#             available = line2.available,
#             active_power_flow = line2.active_power_flow,
#             reactive_power_flow = line2.reactive_power_flow,
#             arc = arc2,
...
#         ),
#         Line(
#             name = "candidate_datacenter_line_3",
#             available = line3.available,
#             active_power_flow = line3.active_power_flow,
#             reactive_power_flow = line3.reactive_power_flow,
#             arc = arc3,
...
#             x = line3.x,
#             b = line3.b,
#             rating = line3.rating*5,
#             angle_limits = line3.angle_limits,
#             ext = Dict(
#                 "is_candidate" => true,
#                 "project_cost" => 70000.0,
#             ),
#         ),
#     ]
...
#     end
    ...
    # new_candidates_transformer = [
    #     PhaseShiftingTransformer(
    #         name = "candidate_transformer_1",
    #         available = transformer1.available,
    #         active_power_flow = transformer1.active_power_flow,
    #         reactive_power_flow = transformer1.reactive_power_flow,
    #         arc = arc4,
    #         r = transformer1.r,
    #         x = transformer1.x,
    #         primary_shunt = transformer1.primary_shunt,
    #         tap = transformer1.tap,
    #         α = transformer1.α,
    #         rating = transformer1.rating,
    #         base_power = transformer1.base_power,
    #         base_voltage_primary = transformer1.base_voltage_primary,
    #         base_voltage_secondary = transformer1.base_voltage_secondary,
    #         phase_angle_limits = transformer1.phase_angle_limits,
    #         ext = Dict(
    #             "is_candidate" => true,
    ...
    #     )
    # ]
    # for transformer in new_candidates_transformer
...
function add_datacenter_data!(sys)
    ...
    # line3 = get_component(Line, sys, "bus-1-bus-2-i_1")
    # arc3 = Arc(; from = get_component(ACBus, sys, "bus-4"), to = get_component(ACBus, sys, "datacenter_bus"))
    # datacenter_line = Line(
    #         name = "datacenter_line",
    #         available = line3.available,
    #         active_power_flow = 0,
    ...
    #         x = line3.x,
    #         b = line3.b,
    #         rating = line3.rating*3,
    #         angle_limits = line3.angle_limits,
    #     ),
    # add_component!(sys, datacenter_line)
    return sys
end

function add_candidate_datacenter_line_data!(sys, scenario)
    line1 = get_component(Line, sys, "bus-4-bus-10-i_7")
    # arc1 = Arc(; from = get_component(ACBus, sys, "datacenter_bus"), to = get_component(ACBus, sys, "bus-10"))
    line2 = get_component(Line, sys, "bus-1-bus-4-i_2")
    # arc2 = Arc(; from = get_component(ACBus, sys, "bus-1"), to = get_component(ACBus, sys, "datacenter_bus"))
    line3 = get_component(Line, sys, "bus-1-bus-2-i_1")
    arc3 = Arc(; from = get_component(ACBus, sys, "bus-4"), to = get_component(ACBus, sys, "datacenter_bus"))
    arc4 = Arc(; from = get_component(ACBus, sys, "bus-4"), to = get_component(ACBus, sys, "dummy_bus"))
    # transformer1 = get_component(PhaseShiftingTransformer, sys, "bus-3-bus-4-i_5")
    # arc4 = Arc(; from = get_component(ACBus, sys, "bus-3"), to = get_component(ACBus, sys, "datacenter_bus"))
        # Create intermediate bus: number = from_bus_number + 100000
    
    if scenario == "parameters"
        candidateline2 = line2
        candidateline3 = line3
        cost2 = 285.4
        cost3 = 285.4
    elseif scenario == "costs"
        candidateline2 = line3
        candidateline3 = line3
        cost2 = 185.5
        cost3 = 285.4
    elseif scenario == "both"
        candidateline2 = line2
        candidateline3 = line3
        cost2 = 185.5
        cost3 = 285.4
    else
        error("Invalid scenario: $scenario. Must be 'parameters' or 'costs' or 'both'.")
    end
    new_candidates_line = [
        # Line(
        #     name = "candidate_datacenter_line_1",
        ...
        Line(
            name = "candidate_datacenter_line_2",
            available = candidateline2.available,
            active_power_flow = candidateline2.active_power_flow,
            reactive_power_flow = candidateline2.reactive_power_flow,
            arc = arc3,
            r = candidateline2.r,
            x = candidateline2.x,
            b = candidateline2.b,
            rating = 10,
            angle_limits = candidateline2.angle_limits,
            ext = Dict(
                "is_candidate" => true,
                "project_cost" => cost2,
            ),
        ...
        Line(
            name = "candidate_datacenter_line_3",
            available = candidateline3.available,
            active_power_flow = candidateline3.active_power_flow,
            reactive_power_flow = candidateline3.reactive_power_flow,
            arc = arc4,
            r = candidateline3.r,
            x = candidateline3.x,
            b = candidateline3.b,
            rating = 10,
            angle_limits = candidateline3.angle_limits,
            ext = Dict(
                "is_candidate" => true,
                "project_cost" => cost3,
            ),
...
function add_candidate_datacenter_line_data_cost!(sys, delta)
    line1 = get_component(Line, sys, "bus-4-bus-10-i_7")
    # arc1 = Arc(; from = get_component(ACBus, sys, "datacenter_bus"), to = get_component(ACBus, sys, "bus-10"))
    line2 = get_component(Line, sys, "bus-1-bus-4-i_2")
    # arc2 = Arc(; from = get_component(ACBus, sys, "bus-1"), to = get_component(ACBus, sys, "datacenter_bus"))
...
function add_candidate_datacenter_line_data_kv!(sys, kv1, kv2)
    arc1 = Arc(; from = get_component(ACBus, sys, "Baker"), to = get_component(ACBus, sys, "datacenter_bus"))
    arc2 = Arc(; from = get_component(ACBus, sys, "Baker"), to = get_component(ACBus, sys, "dummy_bus"))
